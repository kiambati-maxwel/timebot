  ...
  await Timebox.find({}, (err, allTime) => {
    ...
    allTime.forEach(e => {
      ...
      if (dayGraphArray[0] === undefined) {
        for (let i = 0; i <= 6; i++) {
          const yesterday = new Date(new Date().setDate(new Date().getDate() - i)); // console.log(yesterday.getDate());

          allTime.filter(e => {
            return new Date(e.createdAt).getDate() === yesterday.getDate() && new Date(e.createdAt).getMonth() === yesterday.getMonth() && new Date(e.createdAt).getFullYear === yesterday.getFullYear;
          }).forEach(e => {
            timeGraph += e.time;
          });
